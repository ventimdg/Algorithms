...
from scheme_tokens import next_candidate_token
import sys
import os
...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operators = scheme_eval(expr.first, env)
        operand = expr.rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operators, operand, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(name, do_lambda_form(new_expression, env))
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    for i in range(len(expressions)-1):
        if is_false_primitive(scheme_eval(expressions.first, env, True)):
            return False
...
def make_let_frame(bindings, env):
    ...
    list in a let expression: each itpython3 ok -q 15
m must be a list containing a symbol
