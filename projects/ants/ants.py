...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i, distance = self.place, 0
        while i is not beehive:
            if i.bees and self.min_range <= distance <= self.max_range:
            ...
            while place:
                if place.ant:
                    if not hasattr(place.ant, "doubled"):
                            place.ant.damage *= 2
                            setattr(place.ant, "doubled", True)
                    if place.ant.is_container():
                        if place.ant.contained_ant:
                            if not hasattr(place.ant.contained_ant, "doubled"):
                                place.ant.contained_ant.damage *= 2
                                setattr(place.ant.contained_ant, "doubled", True)
                place = place.exit
        else:
