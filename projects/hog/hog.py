...
def more_boar(player_score, opponent_score):
    ...
    newplayer = [int(i) for i in list(str(player_score))]
    newopponent = [int(i) for i in list(str(opponent_score))]
    if len(newopponent) == 1:
        newopponent.insert(0,0)
    if len(newplayer) == 1:
        newplayer.insert(0,0)
    for i in range(0,2):
        if newplayer[i] >= newopponent[i]:
            return False
    return True
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    while playcondition(score0, score1, goal):
        if playcondition(score0, score1, goal):
            score0 += take_turn(strategy0(score0, score1), score1, dice, goal)  
            say = say(score0, score1) 
            while more_boar(score0, score1) and playcondition(score0, score1, goal):
                score0 += take_turn(strategy0(score0, score1), score1, dice, goal)
                say = say(score0, score1)
        if playcondition(score0, score1, goal):
            score1 += take_turn(strategy1(score1, score0), score0, dice, goal)
            say = say(score0, score1)
            while more_boar(score1, score0) and playcondition(score0, score1, goal):
                score1 += take_turn(strategy1(score1, score0), score0, dice, goal)
                say = say(score0, score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        new_ls = last_score
        if who == 0:
            if score0 - last_score > running_high and score0 > last_score:
                print("Player", who, "has reached a new maximum point gain.", score0 - last_score, "point(s)!")
                new_rh = score0 - last_score
            new_ls = score0
        if who == 1:
            if score1 - last_score > running_high and score1 > last_score:
                print("Player", who, "has reached a new maximum point gain.", score1 - last_score, "point(s)!")
                new_rh = score1 - last_score
            new_ls = score1
        return announce_highest(who, new_ls, new_rh)
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    rolls = 0
    for i in range(1, 11):
        average = make_averaged(roll_dice, trials_count = 1000)(i, dice)
        if average > answer:
            answer = average 
            rolls = i
    return rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if more_boar(score + 1, opponent_score):
        return  10
    else:
        return piggypoints_strategy(score, opponent_score, cutoff=9, num_rolls=3)
    # END PROBLEM 12
